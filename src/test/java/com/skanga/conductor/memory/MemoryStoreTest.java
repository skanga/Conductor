...
class MemoryStoreTest {
    ...
    void testMemoryOperations() throws SQLException {
    ...
    void testPlanOperations() throws SQLException {
        ...
        TaskDefinition[] loadedPlan = memoryStore.loadPlan(workflowId);
        assertNotNull(loadedPlan, "Loaded plan should not be null");
        assertEquals(2, loadedPlan.length, "Should have 2 tasks");
        assertEquals("task1", loadedPlan[0].name);
        assertEquals("First task description", loadedPlan[0].description);
        ...
        TaskDefinition[] finalPlan = memoryStore.loadPlan(workflowId);
        assertEquals(3, finalPlan.length, "Updated plan should have 3 tasks");
        assertEquals("task1-updated", finalPlan[0].name, "First task should be updated");
        assertEquals("task3", finalPlan[2].name, "Should have new third task");
    }

    @Test
    @Order(4)
    @DisplayName("Test handling of non-existent data")
    void testNonExistentDataHandling() throws SQLException {
        ...
        TaskDefinition[] nonExistentPlan = memoryStore.loadPlan("non-existent-workflow");
